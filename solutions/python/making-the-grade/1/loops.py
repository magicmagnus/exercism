...
def round_scores(student_scores):
    ...
    return [round(score) for i, score in enumerate(student_scores)]
...
def count_failed_students(student_scores):
    ...
    """
    failed = [score for i, score in enumerate(student_scores) if score <= 40]
    return len(failed)
...
def above_threshold(student_scores, threshold):
    ...
    """
    best = [score for i, score in enumerate(student_scores) if score >= threshold]
...
def letter_grades(highest):
    ...
    ths = [41 + (i * step) for i in range(4)]
    return ths
...
def student_ranking(student_scores, student_names):
    ...
    for i, name in enumerate(student_names):
        res.append(f'{i+1}. {name}: {student_scores[i]}')
